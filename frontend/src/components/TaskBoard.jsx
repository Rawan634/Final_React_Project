    ...
    <div className="task-board-container p-3 flex-grow-1 d-flex flex-column">
      ...
      {(priorityFilter || favoritesFilter) && (
        <div className="filter-indicator mb-3 p-2 d-flex align-items-center">
          ...
          {priorityFilter && (
            <span className={`filter-badge priority-${priorityFilter.toLowerCase()}`}>{priorityFilter} Priority</span>
          )}
          {favoritesFilter && (
      ...
      <div className="kanban-board flex-grow-1 d-flex gap-3 overflow-auto">
        {/* Pending Column */}
        <div className="kanban-column flex-grow-1" onDragOver={handleDragOver} onDrop={() => handleDrop("Pending")}>
        ...
        {/* In Progress Column */}
        <div className="kanban-column flex-grow-1" onDragOver={handleDragOver} onDrop={() => handleDrop("In Progress")}>
          ...
          <div className="column-content p-2 bg-light rounded-bottom h-100">
            {inProgressTasks.length > 0 ? (
              inProgressTasks.map((task) => (
                <TaskCard key={task._id} task={task} onDragStart={handleDragStart} onUpdate={handleTaskUpdate} />
              ))
            ) : (
              <div className="text-center text-muted p-3">
                <FaSadTear />
        ...
        {/* Completed Column */}
        <div className="kanban-column flex-grow-1" onDragOver={handleDragOver} onDrop={() => handleDrop("Completed")}>
