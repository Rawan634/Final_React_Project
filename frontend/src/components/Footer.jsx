  ...
  return (
    <div className="task-section">
        ...
        {filteredTasks.length > 0 ? (
          filteredTasks.map((task, index) => (
            <TaskCard
              ...
              onUpdate={(updatedTask) => dispatch(updateTask({ index, updatedTask }))}
            />
          ))
        ) : (
      ...
      <div className="d-flex justify-content-center gap-3 rounded-4 footer">
        <button className="btn btn-light px-4" onClick={() => setIsModalOpen(true)}>
          ➕ Add Task
        </button>
        <button className="btn btn-danger px-4" onClick={() => dispatch(clearTasks())}>
          🗑️ Delete All
        </button>
      </div>
      ...
      {isModalOpen && (
        <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
          <div className="modal-window" onClick={(e) => e.stopPropagation()}>
            ...
            {warning && <div className="alert alert-warning mb-3">{warning}</div>} {/* Display warning message */}
            <input
              type="text"
              name="title"
